...
| `runtime/boxlang.production.json` | Resolves application paths from the image's `APP_DIR`, enables production caches and JSON console logging without test mappings, and registers the application and ColdBox appender class paths used by root-mode ColdBox resolution. |
