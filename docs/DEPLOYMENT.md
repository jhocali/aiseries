...
| `runtime/boxlang.production.json` | Resolves application paths from the image's `APP_DIR`, enables production caches and JSON console logging without test mappings, and registers source/appender class paths used by the explicit ColdBox config and LogBox resolution. |
