...
| `runtime/boxlang.production.json` | Resolves application paths from the image's `APP_DIR`, enables production caches and JSON console logging without test mappings, and registers ColdBox's appender directory for LogBox's unqualified appender lookup. |
